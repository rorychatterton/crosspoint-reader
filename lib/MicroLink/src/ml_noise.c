...
/* ChaCha20-Poly1305 AEAD from wireguard-lwip (RFC 8439 reference
 * implementation, crypto/refc). Arduino-ESP32's prebuilt mbedTLS has
 * MBEDTLS_CHACHAPOLY_C disabled, so the mbedtls_chachapoly_* API used
 * upstream is unavailable; the refc primitive is compiled in regardless
 * because WireGuard itself uses it. */
 ...
 *
 * Tailscale ts2021 nonce (Go: binary.BigEndian.PutUint64(n[4:], counter)):
 *   12-byte RFC 8439 nonce = 4 zero bytes || 8-byte BIG-endian counter.
 *
 * The wireguard-lwip refc AEAD takes a uint64_t and builds the WireGuard
 * nonce layout (RFC 8439 state words 13..15): 4 zero bytes || 8-byte
 * LITTLE-endian value (chacha20_init: state[13]=0, state[14]=lo32,
 * state[15]=hi32). Since LE64(bswap64(x)) == BE64(x), passing
 * __builtin_bswap64(counter) yields exactly the Tailscale nonce bytes.
 * (MicroLink v1 used the same bswap trick.)
 *
 * Wrappers renamed noise_aead_* to avoid colliding with the global
 * chacha20poly1305_encrypt/_decrypt symbols from wireguard-lwip.
 * ========================================================================== */
...
static int noise_aead_encrypt(const uint8_t *key, uint64_t nonce,
                                ...
                                uint8_t *ciphertext) {
    /* ciphertext layout: encrypted_data(pt_len) + tag(16), same as refc */
...
static int noise_aead_decrypt(const uint8_t *key, uint64_t nonce,
    ...
    /* refc API: src = ciphertext||tag (src_len includes the 16-byte tag),
     * verifies the tag BEFORE decrypting; returns false on MAC mismatch. */
    if (!chacha20poly1305_decrypt(plaintext, ciphertext, ct_len, ad, ad_len,
                                  __builtin_bswap64(nonce), key)) {
        return -1;
    }
    return 0;
