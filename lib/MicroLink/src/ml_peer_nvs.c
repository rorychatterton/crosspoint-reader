...
/* "tbl" held 92-byte entries without the route; a blob under the old key is
 * erased on init rather than parsed with the new layout. (CrossPoint fork.) */
#define PEER_NVS_BLOB_KEY   "tbl2"
#define PEER_NVS_LEGACY_KEY "tbl"
...
typedef struct __attribute__((packed)) {
    ...
    /* Advertised route the target was selected through (0/0 = none); a warm
     * start must reinstall it on the WG peer. (CrossPoint fork.) */
    uint32_t route_ip;              /* 4 bytes */
    uint8_t route_bits;             /* 1 byte */
} peer_nvs_entry_t;                 /* Total: 97 bytes */
...
esp_err_t ml_peer_nvs_init(void) {
    ...
    s_initialized = true;
    if (nvs_erase_key(s_nvs, PEER_NVS_LEGACY_KEY) == ESP_OK) {
        nvs_commit(s_nvs);
        ESP_LOGI(TAG, "Dropped pre-route peer table");
    }
    load_table();
...
esp_err_t ml_peer_nvs_save(const ml_peer_t *peer) {
    ...
    entry.route_ip = peer->route_ip;
    entry.route_bits = peer->route_bits;
...
static void entry_to_peer(const peer_nvs_entry_t *entry, ml_peer_t *p) {
    ...
    p->route_ip = entry->route_ip;
    p->route_bits = entry->route_bits;
    p->active = true;
    p->wg_peer_index = -1;
    ...
    /* Restore endpoints */
    p->endpoint_count = entry->endpoint_count > 2 ? 2 : entry->endpoint_count;
    for (int j = 0; j < p->endpoint_count; j++) {
...
static bool entry_covers_ip(const peer_nvs_entry_t *entry, uint32_t ip) {
    if (entry->vpn_ip == ip) return true;
    if (entry->route_bits == 0) return false;
    uint32_t mask = entry->route_bits >= 32 ? 0xFFFFFFFFu : (0xFFFFFFFFu << (32 - entry->route_bits));
    return (entry->route_ip & mask) == (ip & mask);
}

static int find_slot_by_ip(uint32_t ip) {
    if (!s_initialized || !s_table || ip == 0) return -1;
    for (int i = 0; i < s_table->count; i++) {
        if (s_table->entries[i].vpn_ip != 0 && entry_covers_ip(&s_table->entries[i], ip)) return i;
    }
    return -1;
}

bool ml_peer_nvs_find(uint32_t vpn_ip, ml_peer_t *out) {
    int slot = find_slot_by_ip(vpn_ip);
    if (slot < 0 || !out) return slot >= 0;
    entry_to_peer(&s_table->entries[slot], out);
    return true;
}
...
bool ml_peer_nvs_poison(uint32_t vpn_ip) {
    ...
    s_table->entries[slot].public_key[0] ^= 0x01;
    return flush_table() == ESP_OK;
}
