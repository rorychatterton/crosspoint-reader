...
typedef struct {
  ...
  const char* priority_peer_name; /* Optional MagicDNS short name or FQDN */
  /* Optional second peer selected from the same targeted map fetch: a DNS
   * resolver inside the tailnet that the application queries through the
   * tunnel before its first request. 0 = none. (CrossPoint fork.) */
  uint32_t secondary_peer_ip;
  /* DERP region to report to control as PreferredDERP from the first request
   * (control records it as this node's HomeDERP; peers reply through it).
   * 0 = unknown, use the compile-time default until the map reveals the
   * target peer's region. Persist microlink_get_derp_region() across sessions
   * and pass it back here. (CrossPoint fork.) */
  uint16_t preferred_derp_region;

  /* Warm start (CrossPoint fork): skip the control plane and rebuild the data
   * plane from what the last cold session left behind: our VPN IP and relay
   * region (passed back here) and the NVS peer table, which must hold the
   * priority (and, if set, secondary) peer. Node identity is already in NVS,
   * so the peers still know us. Requires priority_peer_ip. On a cache miss
   * the session ends in ML_STATE_ERROR and the caller falls back to a cold
   * start; a stale cache shows up as a WireGuard handshake that never gets a
   * response, so the caller bounds microlink_wait_peers_ready() tightly. */
  bool warm_start;
  uint32_t warm_self_ip;     /* microlink_get_vpn_ip() of the last cold session */
  uint16_t warm_derp_region; /* microlink_get_derp_region() of that session; 0 = preferred_derp_region */
  ...
  /* Optional coordination server Noise public key. NULL selects Tailscale's
   * production key. Custom control planes and deterministic integration
   * tests can provide their own 32-byte MachinePublic key here. The bytes
   * only need to remain valid for the duration of microlink_init(). */
  const uint8_t* ctrl_server_public_key;
} microlink_config_t;
...
/** Stable diagnostic label for every connection state. */
...
typedef void (*microlink_data_cb_t)(microlink_t* ml, uint32_t src_ip, const uint8_t* data, size_t len, void* user_data);
...
/**
 * @brief Override the coordination server host (Headscale/Ionscale support)
 * @param ml Handle
 * @param host Hostname, or NULL/empty to use the standard Tailscale control
 *             plane. Must be called after microlink_init() and before
 *             microlink_start().
 */
esp_err_t microlink_set_ctrl_host(microlink_t* ml, const char* host);

/**
 * @brief Wait until the WireGuard tunnel to a peer is up
 * @param ml Handle
 * @param vpn_ip Peer VPN IP in host byte order
 * @param timeout_ms Max wait (0 = 15000)
 * @return ESP_OK when the tunnel is up, ESP_ERR_TIMEOUT otherwise
 *
 * Triggers the WG handshake + CallMeMaybe and polls until the peer session
 * is established. Call this before connecting a plain lwIP/BSD/Arduino TCP
 * socket to a 100.64.0.0/10 address; without an established session the
 * connect fails with EHOSTUNREACH.
 */
esp_err_t microlink_wait_peer_ready(microlink_t* ml, uint32_t vpn_ip, uint32_t timeout_ms);

/**
 * @brief Wait until the WireGuard tunnels to several peers are up
 * @param vpn_ips Peer VPN IPs (host byte order), count of them
 * @param timeout_ms Max wait for all of them (0 = 15000)
 * @return ESP_OK when every peer is up, ESP_ERR_TIMEOUT otherwise
 *
...
/** True when the WireGuard session to the peer is established. (CrossPoint fork.) */
bool microlink_peer_is_up(microlink_t* ml, uint32_t vpn_ip);

/**
 * @brief True when the NVS peer table holds a peer whose address or advertised
 * route covers vpn_ip. Usable before microlink_init(); this is the warm-start
 * precondition for a target. (CrossPoint fork.)
 */
bool microlink_peer_cache_contains(uint32_t vpn_ip);

/**
 * @brief Test hook: flip one byte of the cached peer's public key so a warm
 * start's handshake gets no answer. Returns false when the peer is not cached.
 * The next cold session's map overwrites the entry. (CrossPoint fork.)
 */
bool microlink_peer_cache_poison(uint32_t vpn_ip);

/**
 * @brief Log each MicroLink task's stack high-water mark (min free bytes) at
 * ERROR level so it is visible on ERROR-only device builds. Used to right-size
 * ML_TASK_*_STACK_SIZE for the C3, where task stacks are the largest heap
 * consumer during a session. (CrossPoint fork.)
 */
void microlink_log_stack_watermarks(const microlink_t* ml);

/**
 * @brief DERP region this session relays through: the target peer's home
 * region once the map is parsed (else our own). Persist it and pass it as
 * microlink_config_t.preferred_derp_region next time so control learns our
 * HomeDERP before the first handshake. (CrossPoint fork.)
 */
...
typedef void (*microlink_udp_rx_cb_t)(microlink_udp_socket_t* sock, uint32_t src_ip, uint16_t src_port,
 ...
 * @return ESP_OK on success
 */
esp_err_t microlink_udp_send(microlink_udp_socket_t* sock, uint32_t dest_ip, uint16_t dest_port, const void* data,
                             size_t len);
 ...
 */
esp_err_t microlink_udp_recv(microlink_udp_socket_t* sock, uint32_t* src_ip, uint16_t* src_port, void* buffer,
