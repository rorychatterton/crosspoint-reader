...
#include "microlink.h"
#include "ml_config_defaults.h" /* Arduino/PlatformIO: defaults for CONFIG_ML_* absent from sdkconfig.h */
...
/* Task configuration */
#define ML_TASK_NET_IO_STACK ML_TASK_NET_IO_STACK_SIZE
#define ML_TASK_NET_IO_PRIO 7
#define ML_TASK_NET_IO_CORE 0

#define ML_TASK_DERP_TX_STACK ML_TASK_DERP_TX_STACK_SIZE
#define ML_TASK_DERP_TX_PRIO 5
#define ML_TASK_DERP_TX_CORE 0

#define ML_TASK_COORD_STACK ML_TASK_COORD_STACK_SIZE
#define ML_TASK_COORD_PRIO 5
/* Single-core targets (ESP32-C3): pinning to core 1 asserts at runtime.
 * Upstream pinned coord/wg_mgr to core 1 on dual-core chips. */
#ifndef ML_TASK_COORD_CORE
#define ML_TASK_COORD_CORE tskNO_AFFINITY
#endif

#define ML_TASK_WG_MGR_STACK ML_TASK_WG_MGR_STACK_SIZE
#define ML_TASK_WG_MGR_PRIO 7
#ifndef ML_TASK_WG_MGR_CORE
...
/* RAM cut for small tailnets (upstream: 400; each entry queued by value). */
#ifndef ML_PEER_UPDATE_QUEUE_DEPTH
#define ML_PEER_UPDATE_QUEUE_DEPTH 8
#endif
...
/* DERP. Compile-time fallback relay, used whenever no DERPMap was parsed
 * (the targeted map filter never parses one). Overridable with -D so the
 * QEMU harness can point at a local relay. */
#ifndef ML_DERP_REGION
#define ML_DERP_REGION 9 /* Dallas (dfw) */
#endif
#ifndef ML_DERP_HOST
#define ML_DERP_HOST "derp9e.tailscale.com"
#endif
/* The targeted map path never parses the DERPMap, so no relay hostnames are
 * known. Tailscale's official relays follow "derp<N>.tailscale.com" per region
 * N, so when the node's home region is known from the map (Node.HomeDERP)
 * derive the host from it instead of falling back to ML_DERP_HOST. DERP only
 * delivers to peers connected to the same region: relaying through the
 * compile-time fallback (Dallas) from a Sydney-homed node dropped every
 * WireGuard handshake. Set to 0 (the QEMU harness does) to always use
 * ML_DERP_HOST, e.g. a local relay. (CrossPoint fork.) */
#ifndef ML_DERP_USE_REGION_HOST
#define ML_DERP_USE_REGION_HOST 1
#endif
#ifndef ML_DERP_REGION_HOST_FMT
#define ML_DERP_REGION_HOST_FMT "derp%u%c.tailscale.com"
#endif
#ifndef ML_DERP_PORT
...
/* How long the coord task waits for WireGuard init, the selected peers and
 * DERP admission before declaring the session up. A warm start has no control
 * round trips left to hide behind, so it gives up sooner and lets the caller
 * fall back to a cold start. (CrossPoint fork.) */
#define ML_DATA_PLANE_READY_TIMEOUT_MS 15000
#define ML_WARM_READY_TIMEOUT_MS 8000
...
/* Scratch buffer for one decrypted Noise transport frame (ml_coord.c).
 * Upstream hard-coded 65536 (Noise frame max); noise_recv() rejects frames
 * larger than the buffer, so shrinking this only limits max frame size.
 * Accumulated H2 data is capped by ML_H2_BUFFER_SIZE anyway. */
#ifndef ML_H2_FRAME_BUF_SIZE
#define ML_H2_FRAME_BUF_SIZE (16 * 1024)
#endif
...
/* Task-exit acknowledgements: each task sets its bit immediately before
 * vTaskDelete(NULL) so microlink_stop() can wait for confirmed exit
 * instead of a blind delay. Cleared by microlink_start(). */
#define ML_EVT_NET_IO_EXITED BIT9
#define ML_EVT_DERP_TX_EXITED BIT10
#define ML_EVT_COORD_EXITED BIT11
#define ML_EVT_WG_MGR_EXITED BIT12
...
typedef struct {
  ...
  int endpoint_count;
  /* Advertised route (AllowedIPs prefix) through which the priority target is
   * reached; route_bits == 0 when the target is the peer's own address. */
  uint32_t route_ip;
  uint8_t route_bits;
} ml_peer_update_t;
...
typedef struct {
  ...
  int endpoint_count;
  uint16_t derp_region;
  /* Extra allowed prefix installed on the WG peer (see ml_peer_update_t) */
  uint32_t route_ip;
  uint8_t route_bits;
...
/* RAM cut (upstream: 32 regions x 4 nodes). Each node entry is ~130 bytes;
 * a small client only needs its home region plus a few fallbacks. */
#ifndef ML_MAX_DERP_REGIONS
#define ML_MAX_DERP_REGIONS 4
#endif
#ifndef ML_MAX_DERP_NODES
#define ML_MAX_DERP_NODES 1
#endif
...
typedef struct {
  ...
  void *ssl;               /* WOLFSSL* (device: owned exclusively by DERP I/O task) */
  void *ssl_ctx;           /* WOLFSSL_CTX* created per connection */
#else
  mbedtls_ssl_context ssl; /* QEMU/mbedTLS backend (see ml_derp.c) */
  mbedtls_ssl_config ssl_conf;
  mbedtls_entropy_context entropy;
  mbedtls_ctr_drbg_context ctr_drbg;
#endif
  bool tls_inited; /* TLS objects created (freed on disconnect) */
  bool connected;
  uint64_t last_recv_ms; /* For keepalive watchdog */
} ml_derp_conn_t;

/* Bring-up phase durations in ms, filled by the coord and DERP tasks and
 * printed once at CONNECTED as the "[TIMING] control" summary (ERROR level, so
 * it survives the device's ERROR-only log build). (CrossPoint fork.) */
typedef struct {
  uint64_t start_ms; /* microlink_start() */
  uint32_t ctrl_dns_ms;
  uint32_t ctrl_tcp_ms;
  uint32_t noise_ms;
  uint32_t h2_ms;
  uint32_t reg_ms;
  uint32_t map_ms;
  uint32_t derp_dns_ms; /* includes the region node-letter probe */
  uint32_t derp_tcp_ms;
  uint32_t derp_tls_ms;
  uint32_t derp_proto_ms; /* HTTP upgrade + DERP ClientInfo/ServerInfo */
  uint32_t ready_wait_ms; /* coord's wait for WG init, peers and DERP admission */
} ml_bringup_timing_t;
...
struct microlink_s {
  ...
  /* Reserved before queues, sockets, and task stacks can fragment internal
   * RAM. The coord task reuses it for registration and targeted map records. */
  ml_workspace_t coord_workspace;
  ...
  uint16_t derp_home_region; /* Region we relay through (target's home once known) */
  uint16_t target_derp_region; /* Target peer's HomeDERP from the map, 0 = unknown */
  ...
  char last_error[128];     /* Most recent actionable coordination failure */
  ...
  /* Bring-up phase durations of the current session */
  ml_bringup_timing_t timing;
  ...
  char ctrl_host[64];
  uint8_t ctrl_server_public_key[ML_NOISE_KEY_LEN];
...
/* ml_noise.c */
void ml_noise_init(ml_noise_state_t* state, const uint8_t* local_private, const uint8_t* local_public,
...
esp_err_t ml_noise_encrypt(const uint8_t* key, uint64_t nonce, const uint8_t* ad, size_t ad_len,
                           const uint8_t* plaintext, size_t pt_len, uint8_t* ciphertext);
esp_err_t ml_noise_decrypt(const uint8_t* key, uint64_t nonce, const uint8_t* ad, size_t ad_len,
                           const uint8_t* ciphertext, size_t ct_len, uint8_t* plaintext);

/* ml_h2.c */
int ml_h2_build_headers_frame(uint8_t* out, size_t out_size, const char* method, const char* path,
                              const char* authority, const char* content_type, uint32_t stream_id, bool end_stream);
int ml_h2_build_data_frame(uint8_t* out, size_t out_size, const uint8_t* data, size_t data_len, uint32_t stream_id,
                           bool end_stream);
...
/* Cached peer whose own address or advertised route covers vpn_ip. */
bool ml_peer_nvs_find(uint32_t vpn_ip, ml_peer_t* out);
bool ml_peer_nvs_is_initialized(void);
bool ml_peer_nvs_poison(uint32_t vpn_ip);
...
static inline int ml_socket(int domain, int type, int protocol) {
  if (ml_at_socket_is_ready() && (domain == AF_INET || domain == AF_INET6)) return ml_at_socket(domain, type, protocol);
...
static inline ssize_t ml_sendto(int fd, const void* buf, size_t len, int flags, const struct sockaddr* addr,
                                socklen_t addrlen) {
  if (ml_at_socket_is_at_fd(fd)) return ml_at_sendto(fd, buf, len, flags, addr, addrlen);
  return sendto(fd, buf, len, flags, addr, addrlen);
}

static inline ssize_t ml_recvfrom(int fd, void* buf, size_t len, int flags, struct sockaddr* addr, socklen_t* addrlen) {
...
static inline int ml_getaddrinfo(const char* host, const char* svc, const struct addrinfo* hints,
                                 struct addrinfo** res) {
